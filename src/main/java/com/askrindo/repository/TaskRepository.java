...
public interface TaskRepository extends JpaRepository<Task, String> {
    ...
    @Query(nativeQuery = true, value = "select mst_task.* from mst_task inner join mst_release on mst_task.release_id = mst_release.id inner join mst_user on mst_task.assigned_to = mst_user.id inner join mst_project on mst_release.project_id = mst_project.id where mst_task.assigned_to like %:id% and mst_task.status_done like %:statusDone% and mst_release.release_name like %:releaseName% and mst_project.project_name like %:projectName% and to_char(mst_task.est_end_date, 'YYYY-MM-DD') like %:estStartDate% and (to_char(mst_task.est_start_date, 'YYYY-MM-DD') like %:estEndDate% or to_char(mst_task.est_start_date, 'YYYY-MM-DD') is NULL)")
